...
export default function Lobby({ 
  ...
  const [createStep, setCreateStep] = useState(1); // 1: 選擇遊戲, 2: 設定規則
    ...
    <div className="min-h-screen flex flex-col p-6 md:p-12 relative z-10 text-white">
      ...
      {showCreateModal && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/60 backdrop-blur-md animate-in fade-in duration-300">
          <div className="w-full max-w-2xl bg-[#120726]/90 border border-white/10 shadow-[0_20px_60px_rgba(0,0,0,0.8)] rounded-[3.5rem] p-8 md:p-12 relative overflow-hidden animate-in zoom-in-95 duration-300">
            ...
            {createStep === 1 && (
              <div className="grid grid-cols-1 md:grid-cols-2 gap-6 relative z-10 mb-8">
                ...
                {/* 選項：圈圈叉叉 (未解鎖展示用) */}
                <div 
                  className={`p-6 rounded-[2.5rem] border-2 border-white/5 bg-white/[0.02] cursor-not-allowed transition-all duration-300 flex flex-col items-center text-center grayscale opacity-50 relative overflow-hidden`}
                >
                  <div className="absolute top-4 right-4 bg-white/10 px-3 py-1 rounded-full text-[9px] font-black tracking-widest uppercase">建置中</div>
                  <div className="text-6xl drop-shadow-xl mb-4 mt-2">⭕❌</div>
                  <h3 className="text-xl font-bold text-white mb-2">霓虹井字棋</h3>
                  <p className="text-xs text-white/50 leading-relaxed px-2">經典雙人對戰，加入時間限制與道具機制的全新玩法。</p>
                </div>
              </div>
            )}
            ...
            {/* 步驟 2：設定規則 (針對炸彈貓) */}
            {createStep === 2 && (
              <div className="space-y-6 relative z-10 mb-8 animate-in slide-in-from-right-4 duration-300">
                {/* 規則 1: 最大人數 */}
                <div className="bg-white/5 border border-white/10 rounded-[2.5rem] p-6 flex justify-between items-center">
                  <div>
                    <h4 className="font-bold text-lg">房間人數上限</h4>
                    <p className="text-xs text-white/40 mt-1">超過人數將無法加入房間</p>
                  </div>
                  <div className="flex items-center gap-4 bg-black/40 rounded-full p-1 border border-white/5">
                    ...
                    <span className="font-mono font-bold text-xl w-4 text-center">{roomRules.maxPlayers}</span>
                    <button onClick={() => setRoomRules({...roomRules, maxPlayers: Math.min(8, roomRules.maxPlayers + 1)})} className="w-10 h-10 rounded-full bg-white/10 hover:bg-white/20 font-bold">+</button>
                  </div>
                </div>

                {/* 規則 2: 快速模式 Toggle */}
                <div className="bg-white/5 border border-white/10 rounded-[2.5rem] p-6 flex justify-between items-center cursor-pointer" onClick={() => setRoomRules({...roomRules, fastMode: !roomRules.fastMode})}>
                  <div>
                    <h4 className="font-bold text-lg flex items-center gap-2">
                      快速出牌模式 {roomRules.fastMode ? '⚡' : ''}
                    </h4>
                    <p className="text-xs text-white/40 mt-1">開啟後每回合限制 15 秒，超高壓迫感</p>
                  </div>
                  <div className={`w-14 h-8 rounded-full transition-colors relative flex items-center px-1 ${roomRules.fastMode ? 'bg-emerald-500' : 'bg-white/20'}`}>
