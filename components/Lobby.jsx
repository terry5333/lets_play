    ...
    <div className="min-h-screen flex flex-col p-4 md:p-12 relative z-10 text-white animate-in fade-in duration-700">
      ...
      <div className="w-full max-w-6xl mx-auto flex justify-between items-center mb-10 md:mb-16 px-4">
        ...
        <div className="flex items-center gap-3 bg-white/5 backdrop-blur-2xl border border-white/10 p-2 pr-6 rounded-full shadow-xl">
          {isAdmin && <button onClick={() => router.push('/admin')} className="text-[10px] font-black text-rose-500 border border-rose-500/30 px-3 py-1.5 rounded-full bg-rose-500/10 hover:bg-rose-500 hover:text-white transition-all active:scale-95">ADMIN</button>}
          <div className="w-8 h-8 md:w-10 md:h-10 rounded-full overflow-hidden border border-white/10"><img src={user?.photoURL} alt="avatar" className="w-full h-full object-cover" /></div>
          <div className="flex flex-col"><span className="text-xs font-bold leading-tight">{user?.displayName || '無名氏'}</span><span className="text-[9px] text-yellow-400 font-black uppercase tracking-widest">PTS: {myScore}</span></div>
      ...
      <div className="w-full max-w-6xl mx-auto grid grid-cols-1 md:grid-cols-2 gap-6 md:gap-10 mb-12">
        <button onClick={() => { setShowCreateModal(true); setCreateStep(1); }} className="group relative bg-white/[0.03] border border-white/10 rounded-[2.5rem] md:rounded-[3.5rem] p-10 md:p-16 text-left hover:bg-white/[0.05] transition-all overflow-hidden shadow-2xl">
        ...
        <div className="bg-white/[0.03] border border-white/10 rounded-[2.5rem] md:rounded-[3.5rem] p-10 md:p-16 flex flex-col justify-center relative shadow-2xl">
          ...
          <form onSubmit={(e) => { e.preventDefault(); handleJoinRoom(joinInput); }} className="space-y-4">
            <input type="text" placeholder="輸入 4 位房號" value={joinInput} onChange={e=>setJoinInput(e.target.value)} className="w-full bg-black/40 border border-white/10 rounded-full py-6 text-center font-mono text-3xl outline-none focus:border-cyan-500/50 transition-all" />
      ...
      <div className="w-full max-w-6xl mx-auto bg-white/[0.01] border border-white/5 rounded-[2.5rem] md:rounded-[3rem] p-8 md:p-12 backdrop-blur-md">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-5 gap-4">
          {leaderboard.map((p, index) => (
            <div key={p.uid} className={`flex items-center gap-4 p-5 rounded-[2.2rem] border border-white/5 bg-white/[0.02] transition-all hover:scale-105 ${index === 0 ? 'border-yellow-400/20 bg-yellow-400/5' : ''}`}>
              <div className="w-10 h-10 rounded-full overflow-hidden border border-white/10 flex-shrink-0"><img src={p.avatar} alt="avatar" className="w-full h-full object-cover" /></div>
              <div className="flex flex-col truncate"><span className="text-sm font-bold truncate">{p.name}</span><span className="text-[10px] font-mono text-white/30 uppercase">{p.score} PTS</span></div>
            </div>
          ))}
        </div>
      </div>
      ...
      {showCreateModal && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/80 backdrop-blur-xl animate-in fade-in duration-300">
          <div className="w-full max-w-2xl max-h-[90dvh] flex flex-col bg-[#0e0e12] border border-white/10 rounded-[3rem] md:rounded-[4rem] overflow-hidden shadow-2xl animate-in zoom-in-95">
                ...
                <div className="space-y-6 animate-in slide-in-from-right-8 duration-300">
                   ...
                   <div className="flex justify-between items-center bg-white/5 p-6 rounded-[2rem] border border-white/5">
                     ...
                     <div className="flex items-center gap-6 bg-black/40 p-1 rounded-full border border-white/5">
                       <button onClick={()=>setRoomRules({...roomRules, maxPlayers: Math.max(2, roomRules.maxPlayers-1)})} className="w-8 h-8 md:w-10 md:h-10 bg-white/10 hover:bg-white/20 rounded-full font-bold">-</button>
                       <span className="font-mono font-bold text-xl w-6 text-center">{roomRules.maxPlayers}</span>
                       <button onClick={()=>setRoomRules({...roomRules, maxPlayers: Math.min(10, roomRules.maxPlayers+1)})} className="w-8 h-8 md:w-10 md:h-10 bg-white/10 hover:bg-white/20 rounded-full font-bold">+</button>
                     </div>
                   </div>
                   ...
                   {selectedGame === 'boomcat' && (
                    <div className="space-y-4">
                      <div className="bg-white/5 border border-white/10 rounded-[2rem] p-6 flex justify-between items-center cursor-pointer" onClick={() => setRoomRules({...roomRules, fastMode: !roomRules.fastMode})}>
                        <div><h4 className="font-black text-sm md:text-lg">快速模式 {roomRules.fastMode ? '⚡' : ''}</h4></div>
                        <div className={`w-12 h-7 rounded-full transition-colors relative flex items-center px-1 ${roomRules.fastMode ? 'bg-indigo-500' : 'bg-white/20'}`}><div className={`w-5 h-5 rounded-full bg-white transition-transform ${roomRules.fastMode ? 'translate-x-5' : 'translate-x-0'}`}></div></div>
                      </div>
                      <div className="bg-white/5 border border-white/10 rounded-[2rem] p-6 flex justify-between items-center cursor-pointer" onClick={() => setRoomRules({...roomRules, startWithDefuse: !roomRules.startWithDefuse})}>
                        <div><h4 className="font-black text-sm md:text-lg">預發拆除牌 🛡️</h4></div>
                        <div className={`w-12 h-7 rounded-full transition-colors relative flex items-center px-1 ${roomRules.startWithDefuse ? 'bg-emerald-500' : 'bg-white/20'}`}><div className={`w-5 h-5 rounded-full bg-white transition-transform ${roomRules.startWithDefuse ? 'translate-x-5' : 'translate-x-0'}`}></div></div>
                      </div>
                    </div>
                   )}
                   ...
                   {selectedGame === 'drawguess' && (
                    <div className="space-y-4">
                      <div className="bg-white/5 border border-white/10 rounded-[2rem] p-6 flex justify-between items-center">
                        <h4 className="font-black text-emerald-400">遊戲回合數</h4>
                        <div className="flex items-center gap-4 bg-black/40 p-1 rounded-full border border-white/5">
                          <button onClick={()=>setRoomRules({...roomRules, drawRounds: Math.max(1, roomRules.drawRounds-1)})} className="w-8 h-8 bg-white/10 rounded-full font-bold">-</button>
                          <span className="font-mono font-bold text-lg w-6 text-center">{roomRules.drawRounds}</span>
                          <button onClick={()=>setRoomRules({...roomRules, drawRounds: Math.min(5, roomRules.drawRounds+1)})} className="w-8 h-8 bg-white/10 rounded-full font-bold">+</button>
                        </div>
                      </div>
                      <div className="bg-white/5 border border-white/10 rounded-[2rem] p-6 flex justify-between items-center">
                        <h4 className="font-black text-emerald-400">繪畫秒數</h4>
                        <div className="flex items-center gap-4 bg-black/40 p-1 rounded-full border border-white/5">
                          <button onClick={()=>setRoomRules({...roomRules, drawTime: Math.max(30, roomRules.drawTime-30)})} className="w-8 h-8 bg-white/10 rounded-full font-bold">-</button>
                          <span className="font-mono font-bold text-lg w-10 text-center">{roomRules.drawTime}s</span>
                          <button onClick={()=>setRoomRules({...roomRules, drawTime: Math.min(180, roomRules.drawTime+30)})} className="w-8 h-8 bg-white/10 rounded-full font-bold">+</button>
                        </div>
                      </div>
                    </div>
                   )}
                   ...
                   {selectedGame === 'bingo' && (
                    <div className="bg-white/5 border border-white/10 rounded-[2rem] p-6 flex justify-between items-center">
                      <h4 className="font-black text-fuchsia-400">勝利連線數</h4>
                      <div className="flex items-center gap-4 bg-black/40 p-1 rounded-full border border-white/5">
                        <button onClick={()=>setRoomRules({...roomRules, winLines: Math.max(1, roomRules.winLines-1)})} className="w-8 h-8 bg-white/10 rounded-full font-bold">-</button>
                        <span className="font-mono font-bold text-lg w-6 text-center">{roomRules.winLines}</span>
                        <button onClick={()=>setRoomRules({...roomRules, winLines: Math.min(5, roomRules.winLines+1)})} className="w-8 h-8 bg-white/10 rounded-full font-bold">+</button>
                      </div>
                    </div>
                   )}
