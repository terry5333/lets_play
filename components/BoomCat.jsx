...
const CARD_DIC = {
  ...
  attack: { icon: '⚔️', name: '攻擊卡', theme: 'rose', grad: 'from-[#881337] via-[#ef4444] to-[#e11d48]', glow: 'shadow-[0_0_30px_rgba(239,68,68,0.5)]', desc: '強制下一位玩家連續進行兩回合。' },
  ...
  favor: { icon: '🤲', name: '索要卡', theme: 'amber', grad: 'from-[#78350f] via-[#f59e0b] to-[#d97706]', glow: 'shadow-[0_0_30px_rgba(245,158,11,0.5)]', desc: '強制對手給予一張卡牌。' },
  ...
  return (
    <div 
      className={`
        ...
        ${isMyTurn ? 'opacity-100 shadow-[0_15px_30px_rgba(0,0,0,0.4)]' : 'opacity-90'}
      `}
      style={{ 
  ...
  return (
    <div className="min-h-screen bg-gradient-to-b from-[#321c60] via-[#211142] to-[#120726] text-white flex flex-col vibe-font relative overflow-hidden selection:bg-rose-500/20">
      ...
      <div className="flex justify-between items-center p-4 md:p-6 z-10">
        ...
        <div className={`px-6 py-2 rounded-full border shadow-lg transition-colors duration-500 ${isMyTurn ? 'bg-yellow-500/20 border-yellow-400/50 shadow-[0_0_20px_rgba(250,204,21,0.3)]' : 'bg-gradient-to-r from-purple-600 to-indigo-600 border-purple-400/30 shadow-[0_0_20px_rgba(147,51,234,0.3)]'}`}>
          <span className={`font-extrabold tracking-widest text-xs drop-shadow-md transition-colors duration-500 ${isMyTurn ? 'text-yellow-400' : 'text-white'}`}>
            炸彈貓咪：{isMyTurn ? '你的回合' : '激戰中'}
          </span>
        </div>
      ...
      <div className="flex justify-center gap-6 md:gap-12 mt-2 z-10 px-4">
        {gameState.alivePlayers?.filter(uid => uid !== user.uid).map(uid => {
          ...
          return (
            <div key={uid} className={`flex flex-col items-center transition-all duration-300 ${isOpponentTurn ? 'scale-110 opacity-100' : 'opacity-80'}`}>
              <div className={`relative w-16 h-16 md:w-20 md:h-20 rounded-full border-[4px] p-1 shadow-[0_10px_20px_rgba(0,0,0,0.5)] transition-colors duration-300 ${isOpponentTurn ? 'border-yellow-400 bg-[#321c60] shadow-[0_0_25px_rgba(250,204,21,0.6)]' : 'border-indigo-400/50 bg-[#211142]'}`}>
      ...
      <div className="flex-1 flex flex-col items-center justify-center relative z-10 px-4">
        ...
        {/* 提示字眼 */}
        <div className="mb-6 h-10 transition-opacity duration-300">
          {isMyTurn && <h2 className="text-2xl font-black text-yellow-400 drop-shadow-[0_0_15px_rgba(250,204,21,0.5)] animate-pulse tracking-widest">輪到你出牌或摸牌！</h2>}
        </div>
        ...
        <div className="flex gap-4">
          <button onClick={drawCard} disabled={!isMyTurn} className={`px-12 py-4 rounded-[2rem] font-black tracking-widest transition-all duration-300 border-2 ${isMyTurn ? 'bg-gradient-to-b from-yellow-300 to-yellow-500 text-yellow-950 border-yellow-200 shadow-[0_5px_0_#a16207,0_0_30px_rgba(250,204,21,0.3)] active:translate-y-1 active:shadow-none hover:scale-105' : 'bg-white/5 text-white/30 border-white/5 cursor-not-allowed'}`}>
            摸牌結束
          </button>
        </div>
      </div>
      ...
      <div className="relative h-64 md:h-72 w-full flex justify-center items-end pb-8 z-10 px-4">
        {/* 我的頭像 */}
        <div className="absolute left-6 bottom-8 flex flex-col items-center z-20">
          <div className={`w-20 h-20 rounded-full border-4 p-1 bg-[#211142] transition-all duration-300 ${isMyTurn ? 'border-yellow-400 shadow-[0_0_30px_rgba(250,204,21,0.6)] scale-110' : 'border-indigo-400/50 shadow-lg'}`}>
            <img src={user?.photoURL} alt="me" className="w-full h-full object-cover rounded-full" />
          </div>
          <div className={`backdrop-blur-md px-4 py-1.5 rounded-full text-xs font-black mt-3 border transition-colors duration-300 tracking-wide shadow-inner ${isMyTurn ? 'bg-yellow-500/20 border-yellow-500/50 text-yellow-300' : 'bg-purple-900/80 border-purple-500/50 text-white'}`}>
            我 ({myHand.length})
          </div>
        </div>
